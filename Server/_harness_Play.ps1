...
$script:LblSubtitle = New-Lbl 'v0.6.5  Â·  Update 6 - Fix 5  Â·  by Kinan  Â·  Discord: @raed713' $C.dim 9 18
...
# ---------------------------------------------------------------------------------------
# ================= HARNESS DRIVER (test only - never shipped) =================
# Redirect all output to a file so results aren't lost when console truncates
$outputFile = "$env:TEMP\kbf_harness_$($pid).txt"
Start-Transcript -Path $outputFile -Force | Out-Null
$script:HarnessIssues = New-Object System.Collections.Generic.List[string]
$script:Form.Show()
[System.Windows.Forms.Application]::DoEvents()
Start-Sleep -Milliseconds 500
[System.Windows.Forms.Application]::DoEvents()
$script:HarnessG = $script:Form.CreateGraphics()

function Audit-Recurse($Root, [string]$Path) {
    foreach ($c in $Root.Controls) {
        if (-not $c.Visible) { continue }
        $cp = $Path + '\' + $c.GetType().Name
        if ($c.Right -gt ($Root.ClientSize.Width + 2) -or $c.Bottom -gt ($Root.ClientSize.Height + 2)) {
            if ($Root.AutoScroll) { } else {
                $script:HarnessIssues.Add("OUT-OF-BOUNDS $cp right=$($c.Right) bottom=$($c.Bottom) parent=$($Root.ClientSize)")
            }
        }
        if ($c -is [System.Windows.Forms.Label]) {
            if (-not $c.AutoSize -and $c.Text) {
                $s = $script:HarnessG.MeasureString($c.Text, $c.Font, [int][math]::Max(1, $c.Width))
                $lines = [int][math]::Ceiling(($s.Height / [math]::Max(1, $c.Font.Height)) - 0.1)
                if ($c.Height -lt [int]($lines * $c.Font.Height)) {
                    $t = $c.Text; if ($t.Length -gt 45) { $t = $t.Substring(0, 45) + '...' }
                    $script:HarnessIssues.Add("LABEL-CLIP $cp '$t' w=$($c.Width) h=$($c.Height) lines=$lines needs=$([int]($lines * $c.Font.Height))")
                }
            }
        } elseif ($c -is [System.Windows.Forms.Button] -and $c.Text) {
            $tw = $script:HarnessG.MeasureString($c.Text, $c.Font).Width
            if ($tw -gt ($c.Width - 6)) {
                $script:HarnessIssues.Add("BUTTON-CLIP $cp '$($c.Text)' w=$($c.Width) needs=$([int]$tw)")
            }
        }
        Audit-Recurse $c $cp
    }
...
function Run-FitAudit([string]$When) {
    Audit-Recurse $script:Content 'CONTENT'
    Audit-Recurse $script:Chrome.Header 'HEADER'
    Audit-Recurse $script:Chrome.Toolbar 'TOOLBAR'
    Audit-Recurse $script:Chrome.StatusBar 'STATUS'
    Audit-Recurse $script:Chrome.LogPanel 'LOG'
}
...
function Pump-UI {
    ...
    if ($script:PendingAction -and $script:PendingAction.Handle.IsCompleted) {
        $pa = $script:PendingAction
        try { $null = $pa.Ps.EndInvoke($pa.Handle) } catch { }
        $pa.Ps.Dispose()
        $tag = $pa.Tag
        $script:PendingAction = $null
        $script:Busy = $false
...
function Wait-ForState([string]$Name, $Value, [int]$Seconds) {
...
$pages = @(
    @{ N = 'home'; S = { Show-HomePage; Refresh-Dashboard } },
    @{ N = 'fix'; S = { Show-FixPage } },
    @{ N = 'vpn'; S = { Show-VpnPage } },
    @{ N = 'mods'; S = { Show-ModsPage } },
    @{ N = 'settings'; S = { Show-SettingsPage } },
    @{ N = 'guide'; S = { Show-GuidePage } }
)

foreach ($sz in @(@(960, 660), @(1000, 720), @(1440, 900))) {
    $script:Form.Size = New-Object System.Drawing.Size($sz[0], $sz[1])
    foreach ($pg in $pages) {
        & $pg.S
        Layout-Chrome
        if ($script:PageLayout) { & $script:PageLayout }
        ...
        [System.Windows.Forms.Application]::DoEvents()
        $before = $script:HarnessIssues.Count
        Run-FitAudit "$($pg.N)@$($sz[0])x$($sz[1])"
        $newIssues = @($script:HarnessIssues | Select-Object -Skip $before)
        Write-Host ("AUDIT | $($pg.N)@$($sz[0])x$($sz[1]) | issues=$($newIssues.Count)")
        $newIssues | ForEach-Object { Write-Host "  ISSUE: $_" }
    }
}

# Fullscreen regression: maximize on EVERY page with no normal resize first.
$script:Form.Size = New-Object System.Drawing.Size(1000, 720)
foreach ($pg in $pages) {
    & $pg.S
    [System.Windows.Forms.Application]::DoEvents()
    ...
    $before = $script:HarnessIssues.Count
    Run-FitAudit "$($pg.N)@MAX"
    $newIssues = @($script:HarnessIssues | Select-Object -Skip $before)
    Write-Host ("AUDIT | $($pg.N)@MAXIMIZED | issues=$($newIssues.Count)")
    $newIssues | ForEach-Object { Write-Host "  ISSUE: $_" }
...
# The reported breakage sequence: guide -> switch page -> resize
Show-GuidePage; & $script:PageLayout
[System.Windows.Forms.Application]::DoEvents()
Show-SettingsPage; & $script:PageLayout
$script:Form.Size = New-Object System.Drawing.Size(1180, 780)
Layout-Chrome; & $script:PageLayout
[System.Windows.Forms.Application]::DoEvents()
$seqIssues = @($script:HarnessIssues | Select-Object -Skip $before)
Write-Host ("AUDIT | guide-switch-resize | issues=$($seqIssues.Count)")
$seqIssues | ForEach-Object { Write-Host "  ISSUE: $_" }

# Header: subtitle must sit below the title, chip inside the header
$hdr = $script:Chrome.Header
$subBottom = $script:LblSubtitle.Top + $script:LblSubtitle.Height
Check 'header-subtitle-below-title' ($script:LblSubtitle.Top -ge ($script:LblSubtitle.Parent.Controls | Where-Object { $_.Text -eq 'K BNG M Hoster' } | Select-Object -First 1).Bottom) "subtitle.Top=$($script:LblSubtitle.Top)"
Check 'header-subtitle-inside' ($subBottom -le $hdr.ClientSize.Height) "subtitle bottom=$subBottom header=$($hdr.ClientSize.Height)"
Check 'header-chip-inside' ($script:LblVersionChip.Right -le $hdr.ClientSize.Width -and $script:LblVersionChip.Bottom -le $hdr.ClientSize.Height) "chip right=$($script:LblVersionChip.Right) bottom=$($script:LblVersionChip.Bottom) header=$($hdr.ClientSize)"

# Home page: tiles + private note (sandbox config is Private = true)
Show-HomePage; & $script:PageLayout
[System.Windows.Forms.Application]::DoEvents()
$script:State.Conn = Get-ConnectionInfo
Refresh-Dashboard
Pump-UI
$tiles = @($script:TileStart, $script:TileFix, $script:TileVpn, $script:TileMods, $script:TileSettings, $script:TileGuide)
Check 'home-six-tiles' ($tiles.Count -eq 6 -and ($tiles | Where-Object { -not $_ }).Count -eq 0) 'all six tiles exist'
Check 'home-tiles-visible' (($tiles | Where-Object { -not $_.Visible }).Count -eq 0) 'all tiles visible'
$privOk = $script:LblConnPub.Text -match 'private'
Check 'home-private-note' $privOk $script:LblConnPub.Text
Check 'home-cgnat-badge-reset' ($script:LblCgnatBadge.Height -eq 0 -or $script:LblCgnatBadge.Height -gt 0) 'badge exists'

# Fix page: summary + Fix all + new rows after the background scan
$script:State.FixReport = $null
Show-FixPage
[System.Windows.Forms.Application]::DoEvents()
Check 'fix-all-button' [bool]$script:BtnFixAll "exists=$([bool]$script:BtnFixAll)"
$scanDone = Wait-ForState 'fixreport' { $script:State.FixReport } 90
Check 'fix-scan-done' $scanDone 'FixReport arrived'
if ($scanDone) {
    $keys = @($script:State.FixReport | ForEach-Object { $_.Key })
    Check 'fix-rows-new' (($keys -contains 'MODS') -and ($keys -contains 'DISK') -and ($keys -contains 'VER')) "keys=$($keys -join ',')"
}
$summaryOk = Wait-ForState 'fixsummary' { $script:LblFixSummary.Text -match '^Scan finished: \d+ of \d+ checks OK, \d+ need attention\.$' } 30
Check 'fix-summary-line' $summaryOk $script:LblFixSummary.Text

# Settings: visibility radios reflect the sandbox Private = true config
Show-SettingsPage; & $script:PageLayout
[System.Windows.Forms.Application]::DoEvents()
Check 'settings-vis-radios' ([bool]$script:RadioPublic -and [bool]$script:RadioPrivate) 'radios exist'
Check 'settings-vis-preselect' ($script:RadioPrivate.Checked -eq $true -and $script:RadioPublic.Checked -eq $false) "private=$($script:RadioPrivate.Checked) public=$($script:RadioPublic.Checked)"
Check 'settings-map-combo' ($script:CmbMaps.Items.Count -ge 20) "maps=$($script:CmbMaps.Items.Count)"
Check 'settings-map-preselect' ($script:CmbMaps.SelectedIndex -ge 0) "selected=$($script:CmbMaps.SelectedIndex) text=$($script:CmbMaps.Text)"

# Mods: async list population
$script:State.ModsInfo = $null
Show-ModsPage
[System.Windows.Forms.Application]::DoEvents()
$modsOk = Wait-ForState 'modsinfo' { $script:State.ModsInfo } 30
Check 'mods-async-list' $modsOk 'ModsInfo arrived'
if ($modsOk) {
    Check 'mods-enabled-items' ($script:ListEnabled.Items.Count -eq @($script:State.ModsInfo.Enabled).Count) "list=$($script:ListEnabled.Items.Count) state=$(@($script:State.ModsInfo.Enabled).Count)"
}

# Mods multi-select checks
Check 'mods-selection-mode' ($script:ListEnabled.SelectionMode -eq 'MultiExtended' -and $script:ListDisabled.SelectionMode -eq 'MultiExtended') "enabled=$($script:ListEnabled.SelectionMode) disabled=$($script:ListDisabled.SelectionMode)"

# Guide page: no duplicate nav buttons
Show-GuidePage; & $script:PageLayout
$btnCount = 0
foreach ($c in $script:Content.Controls) { foreach ($c2 in $c.Controls) { if ($c2 -is [System.Windows.Forms.Button]) { $btnCount++ } } }
Check 'guide-no-nav-buttons' ($btnCount -eq 0) "buttons on guide page=$btnCount"
Check 'guide-card' ([bool]$script:GuideCard) "card=$($script:GuideCard.Size)"

# Status bar shortcut fit (with Ctrl+H added)
$short = $script:Chrome.LblShortcuts
$m2 = Measure-Text $short.Text $short.Font ([int]$short.Width)
Check 'status-shortcuts-fit' ($m2.Lines -eq 1) "lines=$($m2.Lines) width=$($short.Width)"
Check 'status-shortcuts-home' ($short.Text -match 'Ctrl\+H Home') $short.Text

# Toolbar: all 10 buttons inside the toolbar
$tb = $script:Chrome.Toolbar
$tbOk = $true
foreach ($c in $tb.Controls) { if ($c.Right -gt $tb.ClientSize.Width -or $c.Bottom -gt $tb.ClientSize.Height) { $tbOk = $false } }
Check 'toolbar-buttons-inside' $tbOk "toolbar=$($tb.ClientSize) buttons=$($tb.Controls.Count)"

# Keyboard: Ctrl+H mapped
$keyMapped = $false
foreach ($line in (Get-Content -LiteralPath ($script:CorePath.Replace('HosterCore.ps1', 'Play_BeamMP.ps1')))) {
    if ($line -match "'H' \{ Show-HomePage") { $keyMapped = $true }
}
Check 'keyboard-ctrl-h' $keyMapped 'Ctrl+H -> Show-HomePage'

Write-Host ('TOTAL ISSUES: ' + $script:HarnessIssues.Count)
Stop-Transcript
$script:HarnessG.Dispose()
if ($script:HarnessIssues.Count -gt 0) { exit 1 }
